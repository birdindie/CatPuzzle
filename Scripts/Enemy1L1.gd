...
var velocity := Vector3.ZERO # Velocidade vertical do Inimigo
var is_on_ground := false # Flag para verificar se o Inimigo está no chão
...
func movement(vec: Vector3) -> void:
	if not is_moving:
		is_moving = true
		$AnimationTree.set("parameters/walk/active", true)
		$MoveParticles.emitting = true # Ativar partículas se o Inimigo estiver tentando se mover

		var a := translation
		var b := a + vec * 2
		$tw_m.interpolate_property(self, "translation", a, b, 0.2, Tween.TRANS_EXPO, Tween.EASE_IN_OUT)
		$tw_m.start()
		yield($tw_m, "tween_all_completed")

		is_moving = false
		$MoveParticles.emitting = false # Desativa partículas se o Inimigo estiver parado
